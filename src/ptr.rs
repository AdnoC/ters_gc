...
pub type Gc<'a, T: 'a> = self::inner::Safe<'a, T>;
pub use self::inner::Weak;
pub use self::inner::Safe;
...
mod inner {
    ...
    impl<'a, T: 'a> Safe<'a, T> {
        pub(crate) fn from_raw_nonnull(
            ...
            Gc::to_safe(Gc::from_raw_gcref(GcRef::from_raw_nonnull(ptr, _marker)))
        }
...
// #[cfg(test)]
    ...
    //
    //             let one = Gc::from_safe(one);
    //             let other_one = Gc::from_safe(other_one);
    //             let two = Gc::from_safe(two);
    //             let other_two = Gc::from_safe(other_two);
    //             // Clone
